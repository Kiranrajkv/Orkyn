...
const hubNodes = [
  { label: "Applications", icon: "⬡", color: "#6366F1", glow: "rgba(99,102,241,0.5)",  angle: 315, desc: "Web and mobile apps" },
  { label: "Data",         icon: "◈", color: "#10B981", glow: "rgba(16,185,129,0.5)",  angle: 45,  desc: "Pipelines and analytics" },
  { label: "Integrations", icon: "⟳", color: "#F5A623", glow: "rgba(245,166,35,0.5)",  angle: 135, desc: "APIs and webhooks" },
  { label: "Systems",      icon: "⬢", color: "#EC4899", glow: "rgba(236,72,153,0.5)",  angle: 225, desc: "ERP and CRM" },
];
    ...
    const draw = () => {
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          ctx.fillStyle = prox > 0.05 ? `rgba(245,166,35,${0.08 + prox * 0.6})` : "rgba(0,0,0,0.055)";
          ctx.fill();
        }
      }
...
export function Hero() {
  return (
    ...
    >
      <InteractiveGrid />
      <div className="absolute top-1/4 right-1/3 w-[400px] h-[400px] lg:w-[500px] lg:h-[500px] bg-[#F5A623] opacity-[0.07] rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 left-1/4 w-[300px] h-[300px] lg:w-[400px] lg:h-[400px] bg-indigo-500 opacity-[0.05] rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-24 z-10 w-full">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="space-y-7 lg:space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="#contact"
                className="relative inline-flex items-center gap-2 rounded-full font-bold text-sm text-white px-7 sm:px-8 overflow-hidden group transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_0_32px_rgba(0,0,0,0.3)]"
              ...
              <Link href="#services"
                className="inline-flex items-center justify-center rounded-full font-semibold text-sm px-7 sm:px-8 border border-black/15 bg-white/80 hover:bg-white hover:border-black/30 hover:shadow-[0_4px_20px_rgba(0,0,0,0.08)] hover:-translate-y-0.5 transition-all duration-300 backdrop-blur-sm"
                style={{ height:"48px" }}>
                See Our Services
              </Link>
            </div>
        ...
        {/* Trusted by — static spread, no repeat */}
        <div className="mt-16 lg:mt-20 pt-10 border-t border-border">
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-10 text-center sm:text-left">
            Trusted by growing businesses worldwide
          </p>
          <div className="flex flex-wrap justify-between items-center gap-y-8 gap-x-6">
            {clientLogos.map((logo, i) => (
              <div key={i}
                className="flex flex-col items-center gap-2.5 cursor-default group flex-shrink-0 transition-all duration-300"
              ...
              >
                <Image
                  ...
                  width={logo.w * 1.4}
                  height={logo.h * 1.4}
                  className="h-10 w-auto object-contain opacity-50 group-hover:opacity-100 transition-opacity duration-300"
                />
